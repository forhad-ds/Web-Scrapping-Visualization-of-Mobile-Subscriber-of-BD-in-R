...
gt_plt_bar_stack_custom <- function(gt_object,
                             column = NULL,
                             palette = scales::hue_pal()(5),
                             labels = paste('G', 1:5, sep = ''),
                             labels_collapse = NULL,
                             position = "fill",
                             width = 100,
                             fmt_fn = scales::label_number(scale_cut = cut_short_scale(), trim = TRUE)) {
  stopifnot("Table must be of class 'gt_tbl'" = "gt_tbl" %in% class(gt_object))
  stopifnot("There must be 2 or 5 labels" = (length(labels) %in% c(2:5)))
  stopifnot("There must be 2 or 5 colors in the palette" = (length(palette) %in% c(2:5)))
  ...
  tab_out <- text_transform(
    ...
    fn = function(x) {
        bar_fx <- function(x_val) {
          ...
          #stopifnot("There must be 2, 3, 4 or 5 values" = (n_val %in% c(2:5)))
